        ...
        if (callback == "search-result-tracks") {
                    ...
                    if (track && track.title) {
                        region.append(`
                            <div class="track-row">
                                ...
                                <p>${track.isStream ? "LIVE" : msToReadableTime(track.length)}</p>
                            </div>`);
                    }
                } catch (e) {
                    console.error("Failed to decode track:", tracks[i], e);
                }
            }
            $(".search-result").fadeIn(200);
            $("#search-loader").fadeOut(200);
            $("#search-result-tracks").animate({ scrollTop: 0 }, "slow");
        } else if (callback && callback.startsWith("playlist-page-")) {
            ...
            tracks.forEach((trackId) => {
                try {
                    ...
                    console.error("Failed to decode track card:", trackId, e);
                }
            });
        }
        $("#search-loader").fadeOut(200);
