...
$(document).ready(function () {
    ...
    $(document).on("click", function (e) {
        ...
        if ($target.closest("#focus-search-query").length) {
            $("#search-query").focus()
            return
        }

        if ($target.closest("#search-query").length || $target.closest(".search-result").length) {
            if ($target.closest("#search-query").length) {
                if (player.searchList.length) {
                    $(".search-result").fadeIn(200)
                }
            }
            if ($target.closest("#search-result-tracks .track-row").length || $target.closest("#add-all-tracks-btn").length) {
                $(".search-result").fadeOut(200)
            }
        } else {
