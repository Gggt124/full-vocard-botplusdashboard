...
class User:
    ...
    async def send_to_bot(self, payload: Dict) -> None:
        ...
        LOGGER.debug(f"send_to_bot op={method} guild={self.guild} bot={self.bot}")
        ...
        if self.guild:
            ...
            LOGGER.warning(f"send_to_bot: no guild or bot for user {self.id}, broadcasting op={method}")
            await BotPool.broadcast(payload)
    ...
    async def _listen(self) -> None:
        while True:
            ...
            LOGGER.info(f"User {self.id} sent: op={parsed.get('op')} query={parsed.get('query', '')!r}")
            await self.send_to_bot(parsed)
