...
logger = logging.getLogger("vocard.ipc_methods")
...
async def getTracks(bot: commands.Bot, data: Dict) -> Dict:
    ...
    payload = {"op": "getTracks", "userId": data.get("userId"), "callback": data.get("callback"), "tracks": []}

    if query:
        try:
            node = NodePool.get_node()
            tracks = await node.get_tracks(query=query, requester=None)
            if not tracks:
                return payload

            track_list = tracks.tracks if isinstance(tracks, Playlist) else tracks
            payload["tracks"] = [ track.track_id for track in track_list if track and hasattr(track, 'track_id') ]
            logger.debug(f"getTracks query={query!r} → {len(payload['tracks'])} tracks")
            return payload
        except Exception as e:
            logger.error(f"getTracks failed for query={query!r}", exc_info=e)
            return payload
